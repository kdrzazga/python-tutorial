...
class SleighTest:
    def __init__(self, width=1000, height=700):
        ...
        pygame.display.set_caption("Sleigh Test")
        self._init_gl()
        self.sleigh = Sleigh(0.0, 0.35, 0.0, size=1.0, seed=1)
        seat = self.sleigh.seat_position()
        self.santa = SantaClaus(seat[0], seat[1], seat[2], size=0.8, wave=True, seed=2)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    running = False
            self.santa.update(delta_seconds)
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            glLoadIdentity()
            orbit = self.elapsed * 0.3
            gluLookAt(math.sin(orbit) * 5.6, 2.3, math.cos(orbit) * 5.6, 0.0, 1.05, 0.0, 0.0, 1.0, 0.0)
