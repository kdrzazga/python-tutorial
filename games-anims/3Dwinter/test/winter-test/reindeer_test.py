...
class ReindeerTest:
    def __init__(self, width=1000, height=700):
        ...
        pygame.display.set_caption("Reindeer Test")
        self._init_gl()
        self.reindeer = Reindeer(0.0, 0.0, 0.0, size=1.0, gallop=True, seed=1)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    running = False
            self.reindeer.update(delta_seconds)
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            glLoadIdentity()
            orbit = self.elapsed * 0.3
            gluLookAt(math.sin(orbit) * 5.0, 2.1, math.cos(orbit) * 5.0, 0.0, 1.25, 0.0, 0.0, 1.0, 0.0)
